...
import { BOOKING_STATUS_TABS, type Booking, type BookingStatus } from "./bookingsData";
...
/** Search input on the left, a status filter dropdown on the right. */
export function BookingsToolbar({
  ...
  const activeLabel = BOOKING_STATUS_TABS.find((tab) => tab.status === activeStatus)?.label;
  ...
  return (
    <div className="flex items-center gap-3">
      <SearchInput
        ...
        placeholder="Search bookings by customer, event, or hall..."
      />
      ...
      <div ref={menuRef} className="relative shrink-0">
        ...
        {isOpen && (
          <div className="absolute right-0 z-10 mt-2 w-48 rounded-2xl border border-ink/10 bg-white p-1.5 shadow-lg">
            {BOOKING_STATUS_TABS.map(({ status, label }) => {
